...
``saknussemm.integrations.vision.MultimodalStructuredClient`` so
...
``saknussemm[anthropic|mistral|…]`` is a Phase-5 roadmap item; this is the
...
errors additionally route through :func:`saknussemm.sanitize_error`, which
...
def _raise_for_status(resp: Any, api_key: str) -> None:
    ...
    """
    # Lazy, like every other saknussemm import here: this module is loaded
...
class AnthropicMultimodalClient:
    ...
    async def complete_structured_multimodal(
        ...
        *,
        ...
        from saknussemm.core.schemas import Usage
