...
| **T7** surfaces internes | importeurs cœur · longitudinal · philologie · métriques | 🔨 **partiel** : 3 métriques livrées — `cer_diplo` (#25) · `diacritic_err` (#26) · `del_rate`/`ins_rate` (#27), câblées sur le benchmark réel BNL · **+ `synthesis`** (section verdict factuel par vue, gatée `cer`, déterministe — ferme la dernière case S4) · **+ importeur IIIF** (1ᵉʳ importeur cœur : adapter `_http` anti-SSRF + parser manifeste v2/v3 → service `app` matérialise un `CorpusSpec` images-seules ; sortie unique `CorpusSpec`, transport durci, test loopback réel + `live` distant) · reste importeurs (Gallica/eScriptorium/HTR-United/HF) · longitudinal |
