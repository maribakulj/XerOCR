...
| D-020 | 2026-06-02 | corpus réel (BNL) | Brancher un **vrai** corpus patrimonial (la démo n'avait que 3 docs synthétiques) ? | **Mini-GT BNL** (presse Fraktur luxembourgeoise, données ouvertes) en **fixture déterministe** `tests/fixtures/reference_corpus/bnl_mini/` : GT extraite des ALTO v4 BNL + sorties Tesseract 5.3.4 **figées** (`frk` Fraktur vs `deu` moderne, `--psm 3`) rejouées via `precomputed` → benchmark **CI-safe sans Tesseract**. Builder + 3 tests (`tests/interfaces/test_real_corpus_bnl.py`), 2 vues (`text`, `caseless`). | Premier verdict XerOCR sur **données réelles** : CER frk 0,088 / deu 0,087 (`text`) ; **significativité inter-moteurs VIVANTE** (n=10 ≥ plancher 6 → Wilcoxon calcule : p=1,0/0,91/0,85 → **pas de différence significative** = verdict factuel honnête, plus de « — »). La vue `caseless` montre que `frk` fait surtout des **erreurs de casse** (CER 0,088→0,068). Réalise le « mini-corpus de référence » de `CLAUDE.md §10`. | images PNG + ALTO non committés (poids) → run OCR *live* = test `live` ; tags par-doc (strates) = ajout d'enveloppe futur (T7) ; `by-engine`/`synthesis` restent différés (2 moteurs) |
