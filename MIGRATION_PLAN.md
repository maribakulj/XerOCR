...
| 8 `interfaces` | 🔨 **T4 socle ✅ + Space S1 ✅ · S2 🔨** (vitrine read-only · CLI · coquille design · lanceur run/SSE/Moteurs/upload/gardes ; reste S2.2→S6) | `xerocr/interfaces/ANALYSE_COUCHE_8.md` §DoD |
...
| **S2** lanceur « Banc d'essai » | run de fond annulable · Moteurs `/api/engines` · upload durci · gardes HTTP · SSE · page JS | T2,T3,T4 ✅ | 🔨 reste **S2.2** (page Moteurs + UI upload/sélection **au design**) |
...
| **S2** | lanceur : `JobRunner`+`JobStore` (annulation), `/api/engines`, upload ZIP durci, gardes HTTP, SSE+`Last-Event-ID`, page JS — **reste S2.2** (Moteurs + UI upload au design) | T2,T3,T4 | 2 JobStore→1 (SSE réabsorbé) · annulation réelle · CSRF/mode public/zip-bomb **verts d'abord** | TU2 |
