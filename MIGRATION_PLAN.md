...
| 7 `reports` | ✅ **T2** (overview · `cross_engine` · `compare`) **+ design** (S4.a chrome restylé · S4.b.1a polices Fluxisch Else + OCR-A incorporées data-URI) | `xerocr/reports/ANALYSE_COUCHE_7.md` §DoD |
| 8 `interfaces` | 🔨 **T4 socle ✅ · Space S1·S2·S3 ✅ · S4 🔨** (vitrine · lanceur complet · persistance · rapport au design ; reste S4.b/S5/S6) | `xerocr/interfaces/ANALYSE_COUCHE_8.md` §DoD |
...
| **T5** structure / segmentation | `CanonicalLayout` · fan-out région · métrique `(LAYOUT,LAYOUT)` | ⏳ |
...
| **S4** vues rapport au design | overview/by-engine/by-document/crosses/synthesis | couche 7 ✅ | 🔨 chrome + overview (S4.a) · polices data-URI (S4.b.1a) ✅ · readouts+data-bars (S4.b.1b) · by-document (S4.b.2) ✅ · reste **by-engine/crosses/synthesis** |
...
| D-020 | 2026-06-02 | corpus réel (BNL) | Brancher un **vrai** corpus patrimonial (la démo n'avait que 3 docs synthétiques) + autant de moteurs accessibles que possible ? | **Mini-GT BNL** (presse luxembourgeoise XIXᵉ, **multilingue** allemand Fraktur + français, données ouvertes) en **fixture déterministe** `tests/fixtures/reference_corpus/bnl_mini/` : **30 docs**, GT extraite des ALTO v4 BNL + sorties **Tesseract 5.3.4 figées** rejouées via `precomputed` → **CI-safe sans Tesseract**. **4 moteurs** : `frk` (Fraktur legacy) · `deu` (allemand) · `fra` (français) · `deu_latf` (Fraktur LSTM best, tessdata_best). Builder + 3 tests, 2 vues (`text`/`caseless`). | Verdict XerOCR sur **données réelles** : en `caseless` **`deu_latf` gagne** (CER 0,075) devant `frk` 0,077 ; `fra` dernier (0,111, normal sur corpus mixte). **Significativité Friedman vivante** (n=30 ≥ plancher 6) : text:cer **p=0,072** → **non significatif à 0,05** (verdict honnête, plus de « — »). L'effet **langue** (`fra` gagnant sur les pages FR) est visible au **par-document**. Réalise le « mini-corpus de référence » de `CLAUDE.md §10`. | **Pero/Kraken/docTR : modèles inaccessibles** sous la politique réseau (Google Drive/Zenodo/HuggingFace = 403) → Tesseract seul atteignable (apt+GitHub). Images/ALTO non committés (poids) → run *live* = test `live` ; tags par-doc (strate langue) = enveloppe T7 ; **`by-engine` désormais justifié** (section à construire) ; `synthesis` différé |
