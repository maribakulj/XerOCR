...
| 7 `reports` | ✅ **T2** + **design complet** (S4.a→S4.b : chrome · polices data-URI · readouts/data-bars · by-engine · by-document · crosses) **+ `synthesis`** (T7 : verdict factuel par vue — meilleur pipeline CER · Δ au suivant · significativité, gaté sur `cer`, déterministe) | `xerocr/reports/ANALYSE_COUCHE_7.md` §DoD |
| 8 `interfaces` | 🔨 **T4 socle ✅ · Space S1–S5 ✅ · S6 en cours** (vitrine · lanceur · persistance · vues rapport · sécurité publique ; **S6.a** endpoint import IIIF) | `xerocr/interfaces/ANALYSE_COUCHE_8.md` §DoD |
...
3. **Types domain sans consommateur** (`evaluation.py`) : gardés délibérément comme
   vocabulaire déclaratif, à **assumer consciemment** (garde-fou « pas de consommateur
   = supprimé »). **`ProjectionSpec` n'en fait plus partie** : son premier consommateur
   réel existe (runner + `layout_to_text`, T5/D-031) → réserve **levée**.
