...
| 3 `evaluation` | ✅ **T2** (CER/WER/MER · stats `scipy` · `cross_engine`) **+ T7 philologie/profil** (`cer_diplo` · `diacritic_err` · `del_rate`/`ins_rate` ; `rapidfuzz` en prod) **+ T5 structure** (`region_cer` · loader `LAYOUT`) | `xerocr/evaluation/MIGRATION_COUCHE_3.md` §DoD |
...
| **T5** structure / segmentation | `CanonicalLayout` · fan-out région · métrique `(LAYOUT,LAYOUT)` | 🔨 **squelette livré** (precomputed : LAYOUT régions-seules → fan-out → `region_cer` page → rapport, bout-en-bout) · reste segmenteur réel · mappers ALTO/PAGE↔layout · métriques de détection région · câblage corpus réel |
