...
| 8 `interfaces` | 🔨 **T4f** (CLI + `serve`/vitrine + lanceur + Moteurs + upload + gardes HTTP + SSE + **page « Banc d'essai » interactive TU2.f.1** (JS auto-hébergé, SSE live, CSP `script/connect 'self'`) faits ; reste page Moteurs/UI upload au design TU2.f.2) | `xerocr/interfaces/ANALYSE_COUCHE_8.md` §DoD |
...
### Alias historiques : `TU#` → le plan unique `T#`

**Il n'y a qu'UN seul plan ordonné** — celui ci-dessus (roll-up + §2 séquencement
+ §3 détail), où le web vit dans **T4** (sous-tranches T4a–i). Les `TU#` étaient
une **seconde numérotation** (re-découpe UX du Space, décision de cap) ; **ce
n'est plus un plan parallèle** — juste des **alias historiques** (commits, anciens
docs), donnés ici pour la traçabilité. Le **statut fait foi** dans le roll-up + le
sous-tableau T4 ci-dessus (pas de second ledger).
[`PLAN_SPACE_INTERACTIF.md`](PLAN_SPACE_INTERACTIF.md) = **spec UX/produit**, sans
roadmap concurrente.

| Alias `TU#` | = tranche du plan unique | Couche(s) |
|---|---|---|
| TU1 + TU2 | **T4f** (habillage design + lanceur) | 8 (+ câblage 5/6) |
| TU3 | **T4g** (persistance) | 8 + `adapters/storage` |
| TU4 | **T4h** (vues rapport au design) | 8 |
| TU5 | **T7** (importeurs) | 8 + app/adapters |
| TU6 | **T4i** (durcissement public) | 8 |
| TU7 | **T5** (structure / segmentation, UI incluse) | 1–4 + 8 |

> **Axes restants côté moteur pur** (hors couche 8) : **T5** (structure/
> segmentation) et **T6** (extensibilité tierce, entry-points) — non commencés.
> Le cap a délibérément priorisé le Space interactif (T4) avant T5/T6.
...
T4  SPACE / WEB (couche 8) ─────────────────────────────▶  xerocr serve + app interactive
      │   T4a–e vitrine read-only · T4f habillage design + lanceur (run/SSE/Moteurs/upload/gardes)
      │   T4g persistance (push dépôt) · T4h vues rapport au design · T4i durcissement public
      │   [absorbe l'ex-série « TU1…TU4/TU6 » — alias dans la table T⇄TU du roll-up]
T5  STRUCTURE (segmentation / layout)  ★ ──────────────▶ métriques structurelles  [UI incluse ; ex-TU7]
...
T7+ SURFACES INCRÉMENTALES ────────────────────────────▶  1 importeur / métrique / section par tranche
          corpus IIIF/Gallica/… [importeurs = ex-TU5] · longitudinal · philologie · taxo · économie · Docker/release
```
...
**Sous-tranches de la couche 8 (le Space — absorbe l'ex-série « TU »)** — c'est
ici, dans le plan unique, que vit l'ordre du travail web (plus de roadmap
parallèle ; `PLAN_SPACE_INTERACTIF.md` n'en garde que la **spec UX**) :

| Sous-tranche | Livre | ex-TU | État |
|---|---|---|---|
| **T4a–e** | vitrine read-only + `serve` + Docker/Space | — | ✅ |
| **T4f** | habillage design + lanceur (run de fond annulable · Moteurs `GET /api/engines` · upload corpus durci · gardes HTTP 403/409/404/422 · SSE+`Last-Event-ID` · page « Banc d'essai ») | TU1 + TU2 | ✅ — reste **T4f.2** (UI Moteurs + upload au design) |
| **T4g** | persistance des `RunResult` (push dépôt/Dataset après run) | TU3 | ⏳ |
| **T4h** | vues rapport au design (overview/by-engine/by-document/crosses/synthesis) | TU4 | ⏳ |
| **T4i** | durcissement public (mode public · quotas · rate-limit) | TU6 | 🔨 partiel (CSRF · mode public · quotas upload posés) |
