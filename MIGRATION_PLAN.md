...
| 3 `evaluation` | ✅ **T2** (CER/WER/MER · stats `scipy` · `cross_engine`) **+ T7 philologie/profil** (`cer_diplo` · `diacritic_err` · `del_rate`/`ins_rate` ; `rapidfuzz` en prod) **+ T5 structure** (`region_cer` · loader `LAYOUT` JSON+**ALTO**+**PAGE** · **projecteur `layout_to_text`** + exécution `ProjectionSpec` dans le runner) | `xerocr/evaluation/MIGRATION_COUCHE_3.md` §DoD |
