...
| **P3-b.4 — Boucle prouvée** ✅ (local) | **Prouvée de bout en bout en local** : corpus Dresden curé → `import_curated_corpus` → run **Tesseract `deu` réel** (orchestrateur) → CER vs GT → rapport. CER agrégé **0.819** / WER 0.991 (Tesseract échoue sur le Kurrent — verdict factuel correct), vignettes réelles embarquées, diff GT surligné, 0 trace `xerocr`. *Reste pour la variante « liens HF »* : publication b.3 (vrai SHA/URL). | local ✅ / liens HF = après b.3 |
