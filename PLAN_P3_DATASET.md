...
| **P3-b.3 — Publication** | **Script livré** : `scripts/publish_dataset.py <layout_dir> <repo_id>` (`huggingface_hub`, extra `[huggingface]`) — `create_repo`(exist_ok) + `upload_folder`, affiche l'URL + **révision SHA** + l'appel `import_curated_corpus(base_url, revision)` à coller. **À lancer par toi** (compte HF + `HF_TOKEN` write ; l'env Cinoc bloque HF). | **oui (toi)** |
| **P3-b.4 — Boucle prouvée** ✅ | **Local** : corpus curé → run **Tesseract `deu` réel** → CER 0.819 / WER 0.991 (verdict factuel : Tesseract échoue sur le Kurrent), diff GT surligné. **Remote ✅** : dataset **publié** (`Ma-Ri-Ba-Ku/Cinoc-Dresdner.Hofdiarium.1665-Mscr.Dresd.K.80` @ `db171cc5`), rapport rendu avec `images`/`facsimiles` = **URLs IIIF HF** → fac-similés chargés depuis HF via `<img src>`, **0 octet JPEG embarqué** (saveur « réfs IIIF/HF »). 0 trace `xerocr`. | ✅ (publié + liens HF) |
