...
de Cinoc — exactement ce qu'écrirait un tiers branchant un YOLO. Découvert via
l'entry-point ``cinoc.modules`` déclaré dans ``pyproject.toml``, sans forker.
"""
...
def build_sample_pkg_segmenter(kwargs: Mapping[str, object]) -> PkgSegmenter:
    """``ModuleBuilder`` pointé par l'entry-point ``cinoc.modules``."""
    return PkgSegmenter()
