...
échouent. Picarones écrivait dans ``~/.picarones/`` (inscriptible) ; Cinoc doit
résoudre un emplacement de données **distinct** du dossier rapports.

On fixe ``CINOC_DATA_DIR`` et on vérifie qu'aucun ``history.db`` n'apparaît dans
...
def test_history_db_is_not_written_into_reports_dir(
    ...
    monkeypatch.setenv("CINOC_DATA_DIR", str(tmp_path / "data"))
