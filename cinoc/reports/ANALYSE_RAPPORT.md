...
| **L2-d** | **Vue document loin du canonique + casse en fenêtre étroite** | Canonique « Par document » = **galerie compacte** (cartes vignette+titre+mini-barres). Cinoc drill-in = **image fixe 460px** (`html.py:174`) à gauche + diff **2 colonnes** [GT\|sortie] à droite. Bascule 1 colonne seulement < **860px** (`dd-top2`, `html.py:172`) et < **720px** pour `dd-sbs` (`html.py:190`) → à ~1000–1280px : image(½) + [GT\|sortie] tassés. | Cadre image **non fixe** (max-height en `vh`, ou ratio 40/60) ; **remonter** le point de bascule (empiler image au-dessus du diff dès ~1100px) ; en drill-in étroit, diff **unifié** plutôt que 2 colonnes. |
| **L2-e** *(contenu)* ✅ | **Taxonomie « other » 70–76 %** | C'était le bucket des **vraies substitutions** (misreads), pas un échec de classif → renommé **« substitution »** + libellés FR lisibles (`_CLASS_LABELS`). | Fait — chart pertinent, données clés stables. |
