...
> - **Partie 2 (cible Cinoc)** : **périssable**, chaque verdict est
...
> `cinoc/evaluation/MIGRATION_COUCHE_3.md`, `MIGRATION_PLAN.md` (roll-up +
...
| C10 *(2026-06-11)* | docstring source : searchability suit « la convention Elastic `fuzziness: AUTO` » | Le code applique distance ≤ 2 **uniforme à toutes les longueurs** (l'AUTO d'Elastic réserve 2 aux mots ≥ 6 : 0 si < 3, 1 si 3-5) — Cinoc a hérité du 2 plat (`_SEARCH_MAX_DISTANCE = 2`) + appariement glouton « premier non utilisé ». Effet plafond sur les mots courts (≤ 4 caractères : quasi toujours « retrouvables ») → différences entre moteurs écrasées. Durcissement **pré-1.0** acté (SPEC_HIPE §10). | `cinoc/evaluation/metrics/diagnostics.py:26,45-61` |

> **Leçon transverse C6/C7** : les deux bugs « clés fantômes » sont
> l'impossibilité structurelle que Cinoc élimine **par construction** — payloads
...
Cinoc remplace tout par : **scalaires** = `DocumentMetric` (`Observation`) dans
...
| `metrics/numerical_sequences_hooks.py` (102) | Adaptatif (GT sans séquence → `None`) + agrégateur corpus (recalcule les scores micro, cap `lost_items` à 50/catégorie). | **SUPPRIMER** (plomberie hooks) — la logique adaptative/micro passe dans la métrique + le payload Cinoc. |
...
| `metrics/searchability_hooks.py` (81) | Wrapper hook de `searchability.py` (Levenshtein ≤ 2, multiset). | **SUPPRIMER** — `searchability` est **déjà livré** dans Cinoc (T11). Seule différence : la source expose aussi `missed_tokens` (liste) — sans consommateur prévu côté Cinoc, ne pas porter (« pas de consommateur = supprimé »). |
...
| `metrics/worst_lines.py` (199) | Top-N transversal des pires lignes (re-split des textes du `BenchmarkResult`, fragile après `compact()`). | **SUPPRIMER / NE PAS PORTER** — **déjà couvert** par Cinoc (`DiagnosticsPayload.worst_lines`, T11/D-069, collecté au fil du scoring avec extraits verbatim — design supérieur). |
...
# PARTIE 2 — RÉORGANISATION CIBLE Cinoc (périssable, PROVISOIRE — à confirmer au build)
...
| **4g.1 — export** | adaptateur **JSONL HIPE en couche app** (le JSONL porte les TEXTES GT/raw/system, pas les scores → corpus + `pipeline_outputs` requis) ; `cinoc run --hipe-jsonl` ; c'est lui qui sert le leaderboard — le scorer runtime n'est pas nécessaire à l'interop. |
...
| **Double convention « absent »** (HIPE : vide = erreur max ; Cinoc : `None` = N/A) | 4g | La famille conformité matérialise l'absence en `""` + warning (R-1.8) ; partout ailleurs `None` reste `None` ; les deux documentées côte à côte — sinon les chiffres de conformité divergent du scorer sur run incomplet. |
