...
Ordre de résolution : argument explicite › ``CINOC_DATA_DIR`` › ``/data``
...
def resolve_data_dir(data_dir: Path | str | None = None) -> Path:
    ...
    Argument explicite > ``CINOC_DATA_DIR`` > ``/data`` (si inscriptible) >
    dossier temporaire. Éphémère en dernier recours (la persistance durable est
    obtenue en pointant ``CINOC_DATA_DIR`` / ``/data`` sur un volume stable).
    """
    if data_dir is not None:
        ...
        chosen = _PERSISTENT_MOUNT / "cinoc"
    else:
        chosen = Path(tempfile.mkdtemp(prefix="cinoc-data-"))
