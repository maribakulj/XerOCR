...
``CINOC_METRICS``) : un Space exposé ne fuit pas ses statistiques par défaut. On
...
class RequestMetrics:
    ...
    def render(self) -> str:
        ...
        lines = [
            "# HELP cinoc_requests_total Total HTTP requests handled.",
            "# TYPE cinoc_requests_total counter",
        ]
        with self._lock:
            items = sorted(self._counts.items())
        for (method, status), count in items:
            lines.append(
                f"cinoc_requests_total"
