...
- [x] **T5** : `CanonicalLayout` (payload `LAYOUT`, en `domain`) + mapper **`alto_to_layout`** (`formats/alto/layout_map.py`). — *preuve : `tests/formats/alto/test_alto_layout_map.py`*
- [~] **Différé-par-design** : assembleur inverse `layout→ALTO/PAGE`, mapper `page_to_layout`, lecteur `CANONICAL_DOCUMENT`, projecteur `layout→texte` (couche 3). — *épaississements T5 ultérieurs.*
