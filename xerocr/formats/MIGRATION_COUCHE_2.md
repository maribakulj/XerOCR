...
| **L13** | **Dépendances dures** | (2) | `lxml`, `defusedxml` *(ou durcissement lxml natif)*, `pyyaml`. À déclarer dans `pyproject.toml` + mettre à jour le test de whitelist d'architecture (`formats/` n'importe que `domain` + ces libs ; **jamais** `jiwer`/`rapidfuzz`/un moteur OCR). |
