...
- [x] **T5** : `CanonicalLayout` (payload `LAYOUT`, en `domain`) + mappers **`alto_to_layout`** / **`layout_to_alto`** (`formats/alto/layout_map.py`, ALTO ↔ neutre, round-trip texte octet-stable). — *preuve : `tests/formats/alto/test_alto_layout_map.py`* (l'assemblage en artefact `ALTO_XML` vit en couche 5, `AltoAssembler` : `pipeline` ne peut pas importer `formats`).
- [~] **Différé-par-design** : mapper `page_to_layout`, lecteur `CANONICAL_DOCUMENT`, projecteur `layout→texte` (couche 3). — *épaississements T5 ultérieurs.*
