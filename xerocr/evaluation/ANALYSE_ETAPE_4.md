...
| `metrics/readability.py` (252) | Flesch [0,100] borné + **delta** GT↔hyp ; syllabes = groupes de voyelles consécutives (`aeiouyàâäéèêëîïôöùûüÿæœ`), mot sans voyelle = 1 ; phrases = split `[.!?…]`, min 1. | **GARDER** tel quel (formules publiées, protections div/0 en place). |
...
|---|---|
| **Scalaires** | `numseq_strict`, `numseq_value` — `Observation(value=score_global, weight=n_total)` ; `None` si GT sans séquence. `flesch_delta_fr` / `flesch_delta_en` — `Observation(value=Δ, weight=1)` (macro ; un poids « nombre de mots » serait défendable — trancher au build), `None` si GT < 5 mots ; **la vue choisit la langue** (le runner n'a pas de notion de langue : un corpus FR met `flesch_delta_fr` dans `metric_names`). |
...
| **Payload** | `StructuredDataPayload(kind="structured_data", per pipeline : per_category{n_total, n_strict, n_value, strict_score, value_score, lost_items[:cap]}, readability{n_docs, n_docs_with_delta, delta_mean/median/min/max, n_over/under_normalized(±5), over_normalized_rate})`. Cap `lost_items` repris (50 → à confirmer). |
...
| **Données/packaging** | Regex + table de canonisation des devises = constantes du module (sourcées en docstring) ; coefficients Flesch = constante unique `_FLESCH_COEFFS` **avec citation** (Kandel-Moles 1958 / Flesch 1948) — un YAML pour 6 flottants serait du sur-packaging ; à trancher au build si on uniformise avec les tables 4b. |
| **Tests (valeurs main)** | Flesch : phrase witness calculée à la main avec les deux jeux de coefficients ; syllabation cas limites (mot sans voyelle = 1, `œ`) ; chaque regex : un positif/un négatif par catégorie (« 1515 » oui, « 0999 » non ; « f. 12r » ≠ « f. 12v » ; « an III » = 3) ; adaptatif : GT sans séquence → `None`. |
| **Glossaire** | + `numseq_strict`, `numseq_value`, `flesch_delta_fr`, `flesch_delta_en` (FR/EN). |
| **Budget** | 1 module `metrics/structured_data.py` (détecteurs + 2×2 scalaires) ≈ 350 LOC + payload + section ≈ 150 — sous budget sans découpage. |
...
| **Glossaire** | + 7 entrées (les scalaires ci-dessus). |
...
| Aspect | Cible (PROVISOIRE) |
|---|---|
| **Architecture** | **Un collecteur** `TextualFidelityCollector` (pattern Diagnostics) : `observe(pipeline, ref, hyp)` pendant la passe 1 ; à `build()` : (1) fréquences corpus depuis les réfs collectées → tokens rares (`max_freq=2`) → recall multiset par pipeline (+ échantillon `missed`) ; (2) table de modernisation lexicale (difflib, top-N tokens, variantes, `"∅"`). Résout proprement la « pré-passe corpus » : tout est calculé **après** la passe, sur les représentations déjà normalisées — zéro relecture, zéro mécanique nouvelle dans le runner. (3) **Sur-normalisation** : fonction d'analyse `correction_analysis(view, corpus, pipeline_outputs)` (pattern calibration) qui charge RAW_TEXT (avant) + CORRECTED_TEXT (après) des pipelines 2 étages — co-conçue avec `error_absorption` (4e) : **même extraction avant/après, deux mesures**. |
...
| **Payload** | `TextualFidelityPayload(kind="textual_fidelity", per pipeline : rare_tokens{n_rare_ref, n_recalled, recall, missed[:cap]}, modernization{top_tokens[:cap]{token, n_total, n_modernized, rate, variants}}, over_normalization{score, total_correct_ocr_words, over_normalized_count, passages[:20]{gt, ocr, llm}} — bloc absent si pipeline mono-étage)`. |
| **Section** | `reports/sections/textual_fidelity.py`. |
| **Réparations 🔶** | **R5 — bug C7 tué par construction** (payload typé, la section lit des champs). **R6 — câblage homogène** : un seul canal (collecteur + analyse), plus de calcul en couche rapport. **R7 — casse** : suivre la normalisation de la vue (pas de flag `case_sensitive` interne divergent). |
...
| **Tests** | rare : corpus 3 docs calculé main (hapax/dis, multiplicité min()) ; modernization : « nostre→notre » + délétion `∅` ; over_norm : triplet GT/OCR/LLM main (mot juste dégradé, mot faux corrigé → non compté) ; mono-étage → bloc absent. |
| **Glossaire** | + `rare_token_recall`, `over_normalization` (entrées descriptives même sans scalaire si la section les affiche — règle D-093 : seulement si réellement calculé/affiché). |
...
| **error_absorption** | Fonction d'analyse sur `pipeline_outputs` (avant/après — même helper que 4c/over_norm). Payload `ErrorAbsorptionPayload(kind="error_absorption", per pipeline 2-étages : {n_errors_before/after, n_corrected, n_introduced, n_kept_wrong, correction_rate, introduction_rate, net_improvement, samples[:cap]})`. Affiché dans la section pipeline/diagnostics — à co-livrer avec 4c (même tranche ou tranche jumelle, cf. ordre). |
...
| **Métrique & payload** | `metrics/ner.py` : matching IoU 0.5 greedy `(−score, ref_idx, hyp_idx)` + labels casefold, porté tel quel. Scalaire **`ner_f1`** (micro global, `Observation(value=f1, weight=n_entités_GT)`, `None` si GT sans entité) — jonction `(ENTITIES, ENTITIES)` que le registre type-driven accepte sans modification. Payload `NerPayload(kind="ner", iou_threshold, per pipeline : {precision, recall, f1, per_category{label: PRF+support}, missed[:cap], hallucinated[:cap]})`. Section `reports/sections/ner.py`. |
| **Réparations 🔶** | **R12 — bug C6 tué par construction** (payload typé). **R13 — pas de test circulaire** : valeurs PRF dérivées à la main (3 entités GT, 1 décalée IoU 0.6, 1 label différent, 1 hallucinée). |
...
## Ordre de tranches recommandé (PROVISOIRE)

Chaque tranche = métriques + payload + section + glossaire + tests, **livrée
entièrement** (une sous-étape par session de construction).

| # | Tranche | Pourquoi à cette place |
|---|---|---|
| 1 | **4a** `structured_data` | Zéro dépendance (regex/stdlib), petite, et **établit les deux patterns** que tout le reste recopie : scalaire adaptatif (`None` sans signal GT) + payload-par-pipeline + section. Tranche d'étalonnage idéale. |
| 2 | **4b** `philology` | Même pattern que 4a ; **doit suivre 4a** car la résolution R1 (roman) se joue entre les deux (4a livré sans catégorie roman, 4b la possède — entre les deux tranches, le roman n'est compté nulle part : acceptable et court). Introduit le pattern « tables YAML package-data ». |
| 3 | **4c** `textual_fidelity` | Introduit le collecteur corpus (rare/modernization) et **l'extraction avant/après** (over_norm) que 4e réutilise. |
| 4 | **4e** `inter_engine` + `lines` + longitudinal | Réutilise les comptages taxonomy existants (JS) et le helper avant/après de 4c (error_absorption) ; Pettitt indépendant. La plus « composite » des tranches — peut se scinder en 4e.1 (inter_engine + error_absorption) / 4e.2 (lines) / 4e.3 (Pettitt) si une session sature. |
| 5 | **4f** NER | Verticale autonome (adapter + extra + loader + métrique + section) ; aucune dépendance sur 4a-4e ; placée ici pour garder les patterns couche 3 stabilisés avant d'ouvrir un adapter. Peut permuter avec 4e sans risque. |
| 6 | **4d** robustness + image_quality | **La plus lourde hors couche 3** : whitelist PIL(/numpy), orchestration re-OCR couche 6, surface CLI à arbitrer, sections « courbes ». La garder pour la fin évite de bloquer les familles purement couche 3 derrière des décisions d'orchestration. |
...
| **Alignements bag-of-words surinterprétés** | 4e (oracle, absorption), 4c | Bornes documentées dans payload/section (« borne supérieure, ordre ignoré ») ; convention GT vide → `None` (R10). |
...
| **Constantes magiques re-copiées sans source** | 4d, 4a | Chaque constante éditoriale (500, 30, poids, ±5 Flesch, 0.20, [0.30/0.50/1.00], IoU 0.5, max_freq 2, caps 20/50) listée en docstring avec sa lecture — la revue refuse une constante nue. |
| **Tests circulaires** (le travers du renderer NER source) | toutes | Règle §5.8b déjà en vigueur : valeurs attendues dérivées à la main / référence publiée (jiwer, Pettitt 1979, Flesch/Kandel-Moles), jamais la sortie de Picarones rejouée. |
