...
6. **Liste `C` (hcpr/air) : une seule liste, archaïque uniquement** — accents
   ...
   deux directions visibles en classe taxonomy `diacritic`). Contenu par défaut
   fixé à la tranche 4b (Q4).
...
| **+ `hcpr` / `air`** *(SPEC_HIPE §9, arbitrage n°6)* | `hcpr` = taux de préservation d'une **liste configurable de caractères archaïques** (généralisation paramétrable de `diacritic_err`/`mufi_err` — si factorisation en moteur commun, parité bit-à-bit exigée par test) ; **`air` = l'apport net** (Levchenko 2025) : taux d'occurrences de la liste dans la sortie **absentes de la GT** — détecte la sur-historicisation. **Une seule liste, archaïque uniquement** (ſ ꝛ œ æ ß ⁊ ꝑ… + aͤ oͤ uͤ) ; accents modernes exclus (perte = `diacritic_err` ; deux directions = classe taxonomy). Calculés sur vue `raw`/diplomatique ; pour les pipelines 2 étages, aussi sur l'étage brut (distinguer ce que le correcteur a perdu/inséré de ce que l'OCR avait déjà fait). Contenu par défaut = Q4, à la tranche. |
