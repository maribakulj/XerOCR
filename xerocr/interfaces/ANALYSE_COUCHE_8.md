...
> Maj dans le **même commit** que le code. **Statut : 🔨 T4f (TU2.a+b+c faites)** — vitrine + sécurité HTTP + **`serve`** + packaging Docker/Space (read-only) [T4e] ; **lanceur walking-skeleton (TU2.a)** : `POST/GET/cancel /api/runs` → `JobRunner` (couche 6, thread + annulation coopérative) → `JobStore` (couche 5) → `RunResult` écrit, **CSRF** + **gate mode public** ; **onglet Moteurs (TU2.b)** : `GET /api/engines` (dispo/indispo, sondes injectables) ; **upload corpus (TU2.c)** : `POST/GET /api/corpus` (multipart, CSRF) → `CorpusStore` + `extract_corpus_zip` (couche 6) — **ingestion durcie** : anti-traversal (aplatissement basename + `validated_path`), anti-zip-bomb (octets réels plafonnés), quotas, dédup, liste blanche + magic bytes. Reste T4f : **sélection de moteur au lancement** + 403 cloud/409 indispo HTTP (TU2.d), **SSE** + reprise `Last-Event-ID` (TU2.e), **persistance** (TU3), formulaires UI au design (TU2.f), déploiement HF effectif.
