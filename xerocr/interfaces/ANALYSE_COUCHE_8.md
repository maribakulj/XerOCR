...
> Maj dans le **même commit** que le code. **Statut : 🔨 T4f (TU2.a+b faites)** — vitrine + sécurité HTTP + **`serve`** + packaging Docker/Space (read-only) [T4e] ; **lanceur walking-skeleton (TU2.a)** : `POST/GET/cancel /api/runs` → `JobRunner` (couche 6, thread + annulation coopérative) → `JobStore` (couche 5, en mémoire) → `RunResult` écrit (démo `precomputed`), **CSRF** + **gate mode public** ; **onglet Moteurs (TU2.b)** : `GET /api/engines` — détection runtime dispo/indispo (binaire/SDK/clé, sondes injectables) reflétant le mode public. Reste T4f : **upload** corpus ZIP (TU2.c), **sélection de moteur au lancement** + 403 cloud/409 indispo HTTP (TU2.d), **SSE** + reprise `Last-Event-ID` (TU2.e), **persistance** (TU3), formulaires UI au design (TU2.f), déploiement HF effectif.
