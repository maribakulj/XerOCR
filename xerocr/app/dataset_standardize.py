...
XerOCR**, prêt à publier sur Hugging Face (couche 6, outil de build déterministe).
...
from xerocr.adapters.images import iiif_derivative
from xerocr.domain.errors import XerOCRError
from xerocr.formats.pagexml import parse_pagexml
from xerocr.formats.pagexml.types import PagePage, PageRegion
...
class DatasetStandardizeError(XerOCRError):
...
def _dataset_card(config: StandardizeConfig, n_docs: int) -> str:
    ...
    return (
        ...
        "---\n\n"
        f"# {config.name}\n\n"
        f"{n_docs} pages de vérité-terrain, standardisées pour XerOCR "
