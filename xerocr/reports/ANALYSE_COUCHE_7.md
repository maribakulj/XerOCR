...
- **⚠️ Précondition côté `RunResult` (couche 3 — hors périmètre de cette session)** : le filtre live **ré-exécute l'agrégateur par-moteur déjà défini par le runner** (`MIGRATION_COUCHE_3 §6.2` : générique `safe_mean` / custom struct) sur le **sous-ensemble** → **un seul agrégateur** (serveur = client), pas de réimplémentation. Seule exigence de forme : `RunResult` **persiste les valeurs par-doc** (`DocumentMetric`), pas seulement les agrégats. **Macro/micro par métrique = au codage (§13)** ; le rapport réutilise. (Une métrique micro-pondérée custom porte naturellement ses composantes ; ce n'est **pas** un mandat global « composantes partout ».) → **`MIGRATION_COUCHE_3.md §8`**.
...
| **R14** | **Pivot interactif incohérent** : le filtre live utilise un agrégateur **différent** de celui du runner (2ᵉ implémentation → dérive vs la vue canonique) | golden + test : agrégat filtré == ré-exécution de l'agrégateur runner sur le même sous-ensemble, à plusieurs seuils | **un seul** agrégateur réutilisé (serveur = client, pas de réimplémentation) ; `RunResult` persiste les valeurs par-doc (§2.7-B, `MIGRATION_COUCHE_3 §8`) |
