...
| **R-7 Bugs latents (D-I)** | `Corpus(source=)` eScriptorium ✅ **clos** (sortie `CorpusSpec`, `source` ∈ `metadata` → `TypeError` impossible ; prouvé non-mocké `test_escriptorium_local_server.py`). Reste `selected_indices+1` Gallica (à fermer au portage Gallica). | **Test d'intégration non-mocké** : loopback réel (`http.server`) par source — préféré au `live` quand la source exige auth/instance privée. |
