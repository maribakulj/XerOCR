# Vision — Banc de corpus Cinoc standardisé sur Hugging Face
...
**standardisés pour Cinoc**, publiés sur **Hugging Face**, de sorte que :
...
- les **images soient hébergées** de façon durable (IIIF statique) → les saveurs
  de rapport (cf. `cinoc/reports/DECISION_RAPPORT_INTERACTIF.md`) tombent toutes
  seules ;
...
| **Norme du domaine** | Le benchmark OCR/HTR vit sur des corpus partagés. Un banc Cinoc multi-genres est légitime et différenciant. Importeurs HTR-United/IIIF/Gallica déjà présents. |
...
« Adapté à Cinoc » = définir un **layout canonique** du Dataset :
...
- un **importeur** Cinoc qui lit ce layout depuis HF (revision SHA → `RunManifest`).
