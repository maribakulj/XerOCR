...
| Sous-étape | Contenu | Fichiers touchés |
|---|---|---|
| **3a — Rapport interactif** | **compare 2 runs client-side ✅ (D-081)** (`FileReader`+`JSON.parse`, plafond 50 Mo, bandeau sticky, CSP `/reports/` par hash) · **badges moteur A→E ✅ (D-082)** (helper unique `engine_badges.py`, identité stable entre sections) ; *reste* : hash-router + deeplinks + navigation clavier (ARIA) · formatage des nombres localisé FR/EN · palette daltonien (`?palette=`) | `xerocr/reports/` (`compare.js`/`compare_widget.py` + `engine_badges.py`) ; la voie server-side `reports/compare.py` reste pour `xerocr compare` |
