...
| **3b — Galerie & drill-in** | **drill-in diff caractère surligné GT↔hypothèse ✅ (D-085)** (sur les pires lignes du payload `diagnostics`, server-side) ; *reste* : **galerie miniatures** = **décision d'enveloppe ouverte** (le rapport autonome ne porte aucune image ; embarquer des miniatures data-URI = poids — à trancher) | `xerocr/reports/text_diff.py` + section `diagnostics` ; galerie/lazy à concevoir après décision images |
