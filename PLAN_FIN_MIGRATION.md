...
| Sous-étape | Contenu | Fichiers touchés |
|---|---|---|
| **3a — Rapport interactif** | **compare 2 runs client-side ✅ (D-081)** (`FileReader`+`JSON.parse`, plafond 50 Mo, bandeau sticky des deltas, CSP `/reports/` par hash) ; *reste* : badges moteur A→E (helper unique) · hash-router + deeplinks + navigation clavier (ARIA) · formatage des nombres localisé FR/EN · palette daltonien (`?palette=`) | `xerocr/reports/` (`_assets/compare.js` + `compare_widget.py`) ; la voie server-side `reports/compare.py` reste pour `xerocr compare` |
