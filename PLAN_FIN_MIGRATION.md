...
| **3c — Champs de formulaire** | parité CLI/web : **champ `model` des moteurs OCR ✅ (D-087)** · **`/api/models/{provider}` + suggestions vision ✅ (D-088)** · **preview de normalisation (config YAML custom sans persistance) ✅ (D-089)** ; *reste* : config save/load JSON ; sélecteurs `char_exclude`, profil métrique, toggle expose-ALTO | `xerocr/interfaces/web/routers/` + `app/{models,normalization_preview}.py` + templates ; la construction de spec reste en `app/run_planning` (garde-fou `interfaces` mince) |
