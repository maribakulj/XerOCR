...
| **3c — Champs de formulaire** | parité CLI/web : **champ `model` des moteurs OCR ✅ (D-087, referme gap 2c)** · **`/api/models/{provider}` + suggestions vision ✅ (D-088)** ; *reste* : preview de profil de normalisation (validation YAML custom sans persistance) ; config save/load JSON ; sélecteurs `char_exclude`, profil métrique, toggle expose-ALTO | `xerocr/interfaces/web/routers/` + `app/models.py` + templates ; la construction de spec reste en `app/run_planning` (garde-fou `interfaces` mince) |
