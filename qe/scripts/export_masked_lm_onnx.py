...
Tooling, NOT part of the ``saknussemm[qe]`` extra: it needs the heavy
...
own model behind the same seam:
    ...
    python scripts/export_masked_lm_onnx.py \
        ...
        --out ~/.cache/saknussemm/dalembert-onnx --validate
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--out",
        type=Path,
        default=Path.home() / ".cache" / "saknussemm" / "dalembert-onnx",
    )
